...
function makeMempoolBlock (block) {
  ...
  return makeNode(`<div class="block visible">
                <div class="block-info">
                    ...
                    <div class="mono" style="line-height: 1;">${Math.floor(block.Total)} ECR</div>
                    <span class="timespan">
...
function makeBlockSummary (blockHeight, totalSent, time) {
  return `<div class="block-info">
                ...
                <div class="mono" style="line-height: 1;">${Math.floor(totalSent)} ECR</div>
                <span class="timespan">
...
function makeRewardsElement (subsidy, fee, voteCount, rewardTxId) {
  ...
  // const totalECR = Math.round(pow + fund + fee);
  const totalECR = 1
  return `<div class="block-rewards" style="flex-grow: ${totalECR}">
...
function makeVoteElements (votes) {
  ...
  // totalECR = Math.round(totalECR);
  totalECR = 1
  return `<div class="block-votes" style="flex-grow: ${totalECR}">
                ${voteElements.join('\n')}
            </div>`
}
...
function makeTicketAndRevocationElements (tickets, revocations, blockHref) {
  ...
  // totalECR = Math.round(totalECR);
  totalECR = 1
  return `<div class="block-tickets" style="flex-grow: ${totalECR}">
...
function makeTransactionElements (transactions, blockHref) {
  ...
  // totalECR = Math.round(totalECR);
  totalECR = 1
  return `<div class="block-transactions" style="flex-grow: ${totalECR}">
    ...
    this.tooltipTargets.forEach((tooltipElement) => {
      try {
        ...
        if (data.object === 'Vote') {
          ...
          newContent = `<b>${data.object}</b><br>${data.total} ECR`
        }
