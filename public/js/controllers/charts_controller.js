...
function ticketPriceFunc (data) {
  if (data.t) return zipWindowTvYZ(data.t, data.price, data.count, atomsToECR)
  return zipWindowHvYZ(data.price, data.count, data.window, atomsToECR)
}
...
function percentStakedFunc (data) {
  rawCoinSupply = data.circulation.map(v => v * atomsToECR)
  rawPoolValue = data.poolval.map(v => v * atomsToECR)
...
function circulationFunc (chartData) {
  ...
  var data = map(supplies, (n, i) => {
    ...
    addDough(height)
    inflation.push(yMax)
    return [xFunc(i), supplies[i] * atomsToECR, null]
  })
  ...
  plotGraph (chartName, data) {
    ...
    switch (chartName) {
      case 'ticket-price': // price graph
        ...
        yFormatter = customYFormatter(y => y.toFixed(8) + ' ECR')
        break
      ...
      case 'stake-participation':
        ...
        yFormatter = (div, data, i) => {
          ...
          div.appendChild(legendEntry(`${legendMarker()} Ticket Pool Value: ${intComma(rawPoolValue[i])} ECR`))
          div.appendChild(legendEntry(`${legendMarker()} Coin Supply: ${intComma(rawCoinSupply[i])} ECR`))
        }
        break

      case 'ticket-pool-value': // pool value graph
        d = zip2D(data, data.poolval, atomsToECR)
        assign(gOptions, mapDygraphOptions(d, [xlabel, 'Ticket Pool Value'], true,
          'Ticket Pool Value (ECR)', true, false))
        yFormatter = customYFormatter(y => intComma(y) + ' ECR')
        break
      ...
      case 'coin-supply': // supply graph
        ...
        assign(gOptions, mapDygraphOptions(d.data, [xlabel, 'Coin Supply', 'Inflation Limit'],
          true, 'Coin Supply (ECR)', true, false))
        gOptions.series = {
          'Inflation Limit': {
            strokePattern: [5, 5],
            color: '#888',
        ...
        yFormatter = (div, data, i) => {
          addLegendEntryFmt(div, data.series[0], y => intComma(y) + ' ECR')
          var change = 0
          if (i < d.inflation.length) {
            ...
            div.appendChild(legendEntry(`${legendMarker()} Unminted: ${intComma(unminted)} ECR (${change}%)`))
          }
        }
        break

      case 'fees': // block fee graph
        d = zip2D(data, data.fees, atomsToECR)
        assign(gOptions, mapDygraphOptions(d, [xlabel, 'Total Fee'], false, 'Total Fee (ECR)', true, false))
        break
