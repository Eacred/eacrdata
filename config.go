...
import (
	...
	"github.com/Eacred/eacrdata/netparams"
	"github.com/Eacred/eacrdata/version"
...
var (
	...
	defaultPoliteiaAPIURl    = "https://proposals.eacred.org"
...
type config struct {
	...
	HomeDir      string `short:"A" long:"appdata" description:"Path to application home directory" env:"ECRDATA_APPDATA_DIR"`
	ConfigFile   string `short:"C" long:"configfile" description:"Path to configuration file" env:"ECRDATA_CONFIG_FILE"`
	DataDir      string `short:"b" long:"datadir" description:"Directory to store data" env:"ECRDATA_DATA_DIR"`
	LogDir       string `long:"logdir" description:"Directory to log output." env:"ECRDATA_LOG_DIR"`
	...
	OutFolder    string `short:"f" long:"outfolder" description:"Folder for file outputs" env:"ECRDATA_OUT_FOLDER"`
	ShowVersion  bool   `short:"V" long:"version" description:"Display version information and exit"`
	TestNet      bool   `long:"testnet" description:"Use the test network (default mainnet)" env:"ECRDATA_USE_TESTNET"`
	SimNet       bool   `long:"simnet" description:"Use the simulation test network (default mainnet)" env:"ECRDATA_USE_SIMNET"`
	DebugLevel   string `short:"d" long:"debuglevel" description:"Logging level {trace, debug, info, warn, error, critical}" env:"ECRDATA_LOG_LEVEL"`
	Quiet        bool   `short:"q" long:"quiet" description:"Easy way to set debuglevel to error" env:"ECRDATA_QUIET"`
	HTTPProfile  bool   `long:"httpprof" short:"p" description:"Start HTTP profiler." env:"ECRDATA_ENABLE_HTTP_PROFILER"`
	HTTPProfPath string `long:"httpprofprefix" description:"URL path prefix for the HTTP profiler." env:"ECRDATA_HTTP_PROFILER_PREFIX"`
	CPUProfile   string `long:"cpuprofile" description:"File for CPU profiling." env:"ECRDATA_CPU_PROFILER_FILE"`
	UseGops      bool   `short:"g" long:"gops" description:"Run with gops diagnostics agent listening. See github.com/google/gops for more information." env:"ECRDATA_USE_GOPS"`
	ReloadHTML   bool   `long:"reload-html" description:"Reload HTML templates on every request" env:"ECRDATA_RELOAD_HTML"`

	// API
	APIProto            string  `long:"apiproto" description:"Protocol for API (http or https)" env:"ECRDATA_ENABLE_HTTPS"`
	APIListen           string  `long:"apilisten" description:"Listen address for API" env:"ECRDATA_LISTEN_URL"`
	...
	UseRealIP           bool    `long:"userealip" description:"Use the RealIP middleware from the pressly/chi/middleware package to get the client's real IP from the X-Forwarded-For or X-Real-IP headers, in that order." env:"ECRDATA_USE_REAL_IP"`
	CacheControlMaxAge  int     `long:"cachecontrol-maxage" description:"Set CacheControl in the HTTP response header to a value in seconds for clients to cache the response. This applies only to FileServer routes." env:"ECRDATA_MAX_CACHE_AGE"`
	InsightReqRateLimit float64 `long:"insight-limit-rps" description:"Requests/second per client IP for the Insight API's rate limiter." env:"ECRDATA_INSIGHT_RATE_LIMIT"`
	...
	// Data I/O
	MempoolMinInterval int    `long:"mp-min-interval" description:"The minimum time in seconds between mempool reports, regardless of number of new tickets seen." env:"ECRDATA_MEMPOOL_MIN_INTERVAL"`
	MempoolMaxInterval int    `long:"mp-max-interval" description:"The maximum time in seconds between mempool reports (within a couple seconds), regardless of number of new tickets seen." env:"ECRDATA_MEMPOOL_MAX_INTERVAL"`
	MPTriggerTickets   int    `long:"mp-ticket-trigger" description:"The number minimum number of new tickets that must be seen to trigger a new mempool report." env:"ECRDATA_MP_TRIGGER_TICKETS"`
	AgendasDBFileName  string `long:"agendadbfile" description:"Agendas DB file name (default is agendas.db)." env:"ECRDATA_AGENDAS_DB_FILE_NAME"`
	ProposalsFileName  string `long:"proposalsdbfile" description:"Proposals DB file name (default is proposals.db)." env:"ECRDATA_PROPOSALS_DB_FILE_NAME"`
	PoliteiaAPIURL     string `long:"politeiaurl" description:"Defines the root API politeia URL (defaults to https://proposals.eacred.org)."`
	...
	FullMode       bool          `long:"pg" description:"Run in \"Full Mode\" mode,  enables postgresql support" env:"ECRDATA_ENABLE_FULL_MODE"`
	PGDBName       string        `long:"pgdbname" description:"PostgreSQL DB name." env:"ECRDATA_PG_DB_NAME"`
	PGUser         string        `long:"pguser" description:"PostgreSQL DB user." env:"ECRDATA_POSTGRES_USER"`
	PGPass         string        `long:"pgpass" description:"PostgreSQL DB password." env:"ECRDATA_POSTGRES_PASS"`
	PGHost         string        `long:"pghost" description:"PostgreSQL server host:port or UNIX socket (e.g. /run/postgresql)." env:"ECRDATA_POSTGRES_HOST_URL"`
	...
	DisabledExchanges string `long:"disable-exchange" description:"Exchanges to disable. See /exchanges/exchanges.go for available exchanges. Use a comma to separate multiple exchanges" env:"ECRDATA_DISABLE_EXCHANGES"`
	ExchangeCurrency  string `long:"exchange-currency" description:"The default bitcoin price index. A 3-letter currency code" env:"ECRDATA_EXCHANGE_INDEX"`
	RateMaster        string `long:"ratemaster" description:"The address of a ECRRates instance. Exchange monitoring will get all data from a ECRRates subscription." env:"ECRDATA_RATE_MASTER"`
...
// netName returns the name used when referring to a eacred network. TestNet3
