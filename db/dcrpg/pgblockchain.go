...
func (pgb *ChainDB) AddressHistory(address string, N, offset int64,
	...
	log.Infof("%s: %d spent totalling %f ECR, %d unspent totalling %f ECR",
...
// GetSDiff gets the stake difficulty in ECR for a given block height.
...
// GetSDiffRange gets the stake difficulties in ECR for a range of block heights.
...
func (pgb *ChainDB) GetExplorerBlock(hash string) *exptypes.BlockInfo {
	...
	for _, tx := range data.RawSTx {
		...
		case stake.TxTypeSSGen:
			...
			// well.
			if stx.Fee > 0 {
				log.Tracef("Vote with fee! %v, %v ECR", stx.Fee, stx.Fees)
			}
