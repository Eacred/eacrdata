...
const (
	...
	defaultECRRatesPort = "7778"

	aggregatedOrderbookKey = "aggregated"
	orderbookKey           = "depth"
)

var grpcClient dcrrates.ECRRatesClient
...
type ExchangeBot struct {
	...
	// BtcIndex is the (typically fiat) currency to which the ECR price should be
	...
	// exchanges or no up-to-date Eacred exchanges. IsFailed is a getter for failed.
	failed bool
}

// ExchangeBotState is the current known state of all exchanges, in a certain
// base currency, and a volume-averaged price and total volume in ECR.
...
func NewExchangeBot(config *ExchangeBotConfig) (*ExchangeBot, error) {
	...
	if len(bot.DcrBtcExchanges) == 0 {
		return nil, fmt.Errorf("no ECR-BTC exchanges were initialized")
	}
			...
			go func() {
				for {
					...
					if err != nil {
						...
						log.Errorf("ECRRates error. Attempting to reconnect in 10 seconds: %v", err)
						...
						for {
							...
							if err == nil {
								...
								log.Errorf("Failed to reconnect to ECRRates. Attempting to reconnect in %s: %v", delayString, err)
							}
						}
						log.Infof("ECRRates connection re-established.")
						reconnectionAttempt = 0
						continue
					}
...
// Attempt ECRRates connection after delay.
func (bot *ExchangeBot) connectMasterBot(ctx context.Context, delay time.Duration) (dcrrates.ECRRates_SubscribeExchangesClient, error) {
	if bot.masterConnection != nil {
		bot.masterConnection.Close()
	}
	if delay > 0 {
	...
	grpcClient := dcrrates.NewECRRatesClient(conn)
...
// updateExchange processes an update from a Eacred-BTC Exchange.
...
// sources or no Eacred Exchanges are up-to-date. Individual exchanges can
// be outdated/failed without IsFailed being false, as long as there is at least
// one Bitcoin index and one Eacred exchange.
...
// Conversion is a representation of some amount of ECR in another index.
...
func (bot *ExchangeBot) QuickSticks(token string, rawBin string) ([]byte, error) {
	...
	if !found {
		return nil, fmt.Errorf("Failed to find ECR exchange state for %s", token)
	}
