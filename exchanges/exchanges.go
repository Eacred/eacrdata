...
var (
	...
	BinanceURLs = URLs{
		Price: "https://api.binance.com/api/v1/ticker/24hr?symbol=ECRBTC",
		// Binance returns a maximum of 1000 depth chart points. This seems like it
		// is the entire order book at least sometimes.
		Depth: "https://api.binance.com/api/v1/depth?symbol=ECRBTC&limit=1000",
		Candlesticks: map[candlestickKey]string{
			hourKey:  "https://api.binance.com/api/v1/klines?symbol=ECRBTC&interval=1h",
			dayKey:   "https://api.binance.com/api/v1/klines?symbol=ECRBTC&interval=1d",
			monthKey: "https://api.binance.com/api/v1/klines?symbol=ECRBTC&interval=1M",
		},
	}
	BittrexURLs = URLs{
		...
		Candlesticks: map[candlestickKey]string{
			hourKey: "https://bittrex.com/api/v2.0/pub/market/GetTicks?marketName=BTC-ECR&tickInterval=hour",
			dayKey:  "https://bittrex.com/api/v2.0/pub/market/GetTicks?marketName=BTC-ECR&tickInterval=day",
		},
	...
	PoloniexURLs = URLs{
		...
		Depth: "https://poloniex.com/public?command=returnOrderBook&currencyPair=BTC_ECR&depth=100",
		Candlesticks: map[candlestickKey]string{
			halfHourKey: "https://poloniex.com/public?command=returnChartData&currencyPair=BTC_ECR&period=1800&start=0&resolution=auto",
			dayKey:      "https://poloniex.com/public?command=returnChartData&currencyPair=BTC_ECR&period=86400&start=0&resolution=auto",
		},
...
// DcrExchanges maps tokens to constructors for ECR-BTC exchanges.
...
// opposed to a Eacred-to-Bitcoin Exchange.
func IsBtcIndex(token string) bool {
	_, ok := BtcIndices[token]
	return ok
}

// IsDcrExchange checks whether the given token is a known Eacred-BTC exchange.
...
// guaranteed is a price. For Eacred exchanges, the volumes will also be
// populated.
...
type HuobiCandlestickPt struct {
	...
	Vol    float64 `json:"vol"`    // Volume ECR
	Count  int64   `json:"count"`
}
...
// sporadically, but it is used. For the BTC_ECR endpoint, almost all updates
