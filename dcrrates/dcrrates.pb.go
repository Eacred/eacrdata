...
// ECRRatesClient is the client API for ECRRates service.
//
// For semantics around ctx use and closing/ending streaming RPCs, please refer to https://godoc.org/google.golang.org/grpc#ClientConn.NewStream.
type ECRRatesClient interface {
	SubscribeExchanges(ctx context.Context, in *ExchangeSubscription, opts ...grpc.CallOption) (ECRRates_SubscribeExchangesClient, error)
}
...
func NewECRRatesClient(cc *grpc.ClientConn) ECRRatesClient {
	return &dCRRatesClient{cc}
}

func (c *dCRRatesClient) SubscribeExchanges(ctx context.Context, in *ExchangeSubscription, opts ...grpc.CallOption) (ECRRates_SubscribeExchangesClient, error) {
	stream, err := c.cc.NewStream(ctx, &_ECRRates_serviceDesc.Streams[0], "/dcrrates.ECRRates/SubscribeExchanges", opts...)
	if err != nil {
		return nil, err
	}
...
type ECRRates_SubscribeExchangesClient interface {
...
// ECRRatesServer is the server API for ECRRates service.
type ECRRatesServer interface {
	SubscribeExchanges(*ExchangeSubscription, ECRRates_SubscribeExchangesServer) error
}

func RegisterECRRatesServer(s *grpc.Server, srv ECRRatesServer) {
	s.RegisterService(&_ECRRates_serviceDesc, srv)
}

func _ECRRates_SubscribeExchanges_Handler(srv interface{}, stream grpc.ServerStream) error {
	m := new(ExchangeSubscription)
	if err := stream.RecvMsg(m); err != nil {
		return err
	}
	return srv.(ECRRatesServer).SubscribeExchanges(m, &dCRRatesSubscribeExchangesServer{stream})
}

type ECRRates_SubscribeExchangesServer interface {
...
var _ECRRates_serviceDesc = grpc.ServiceDesc{
	ServiceName: "dcrrates.ECRRates",
	HandlerType: (*ECRRatesServer)(nil),
	Methods:     []grpc.MethodDesc{},
	Streams: []grpc.StreamDesc{
		{
			StreamName:    "SubscribeExchanges",
			Handler:       _ECRRates_SubscribeExchanges_Handler,
			ServerStreams: true,
		},
	},
