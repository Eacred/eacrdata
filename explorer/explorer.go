...
import (
	...
	"github.com/Eacred/eacrdata/exchanges"
	"github.com/Eacred/eacrdata/explorer/types"
	"github.com/Eacred/eacrdata/gov/agendas"
	pitypes "github.com/Eacred/eacrdata/gov/politeia/types"
	"github.com/Eacred/eacrdata/mempool"
	pstypes "github.com/Eacred/eacrdata/pubsub/types"
...
var explorerLinks = &links{
	CoinbaseComment: "https://github.com/Eacred/eacrd/blob/2a18beb4d56fe59d614a7309308d84891a0cba96/chaincfg/genesis.go#L17-L53",
	POSExplanation:  "https://docs.eacred.org/proof-of-stake/overview/",
	APIDocs:         "https://github.com/Eacred/eacrdata#apis",
	InsightAPIDocs:  "https://github.com/Eacred/eacrdata/blob/master/api/Insight_API_documentation.md",
	Github:          "https://github.com/Eacred/eacrdata",
	License:         "https://github.com/Eacred/eacrdata/blob/master/LICENSE",
	NetParams:       "https://github.com/Eacred/eacrd/blob/master/chaincfg/params.go",
	DownloadLink:    "https://eacred.org/downloads/",
}
...
func (exp *explorerUI) Store(blockData *blockdata.BlockData, msgBlock *wire.MsgBlock) error {
	...
	// If BlockData contains non-nil PoolInfo, compute actual percentage of ECR
	// supply staked.
	stakePerc := 45.0
	...
	// https://docs.eacred.org/advanced/navigating-politeia-data/#voting-and-comment-data
...
func (exp *explorerUI) simulateASR(StartingECRBalance float64, IntegerTicketQty bool,
	...
	MaxCoinSupplyAtBlock := func(blocknum float64) float64 {
		// 4th order poly best fit curve to Eacred mainnet emissions plot.
	...
	ECRBalance := StartingECRBalance

	ReturnTable += fmt.Sprintf("\n\nBLOCKNUM        ECR  TICKETS TKT_PRICE TKT_REWRD  ACTION\n")
	ReturnTable += fmt.Sprintf("%8d  %9.2f %8.1f %9.2f %9.2f    INIT\n",
		int64(simblock), ECRBalance, TicketsPurchased,
	...
	for simblock < (BlocksPerYear + CurrentBlockNum) {
		...
		if IntegerTicketQty {
			...
			TicketsPurchased = math.Floor(ECRBalance / TicketPrice)
		} else {
			// Use this to simulate ALL funds used to buy tickets - even fractional tickets
			// which is actually not possible
			TicketsPurchased = (ECRBalance / TicketPrice)
		}

		ECRBalance -= (TicketPrice * TicketsPurchased)
		...
		// Simulate reward
		ECRBalance += (StakeRewardAtBlock(simblock) * TicketsPurchased)
		TicketsPurchased = 0
	...
	SimulationReward := ((ECRBalance - StartingECRBalance) / StartingECRBalance) * 100
...
// mempoolTime is the TimeDef that the transaction was received in ECRData, or
